...
async function checkedText(url: string, context: CollectContext, accept = 'text/html,application/xhtml+xml,application/xml;q=0.9,*/*;q=0.7'): Promise<{ response: Response; text: string }> {
  const response = await fetchWithTimeout(url, {
    ...
    headers: {
      'accept': accept,
      'user-agent': 'HotTopicsObservatory/1.0 (+https://github.com/siner9586/Hot_Topics)'
    }
  }, 12_000);
...
export class WikimediaTopAdapter extends BaseAdapter {
  ...
  async collect(context: CollectContext): Promise<RawHotItem[]> {
    ...
    const response=await fetchWithTimeout(url,{...(context.signal?{signal:context.signal}:{}),headers:{'user-agent':'HotTopicsObservatory/1.0 (siner9586/Hot_Topics)','accept':'application/json'}},12_000);
...
export function createAdapters(env: Record<string,string|undefined> = {}): SourceAdapter[] {
  ...
  const adapters:SourceAdapter[]=[
    ...
    Object.assign(new YouTubePopularAdapter(),{enabled:flags.youtube}),
    ...
    Object.assign(new RequiresAccessAdapter('weibo','微博热搜','CN','social',0.9,'1.0.0','disabled','no stable authorized production API configured'),{enabled:flags.weibo}),
    ...
    Object.assign(new RequiresAccessAdapter('gdelt-cloud','GDELT Cloud','GLOBAL','news',0.9,'1.0.0','auth_required','GDELT Cloud API key required for unbiased production story ranking'),{enabled:flags.gdelt})
  ];
  return adapters;
}
